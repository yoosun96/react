...
function createPage(root, stateScript) {
    ...
    <head>
      ...
      <mata
       ...
       />
       <mata name="theme-color" content="#000000" />
       <title>React</title>
    ...
    <body>
       ...
       ${stateScript}
       <script src="${manifest.files['runtime-main.js']}></sciprt>
         ${chunks}
...
const serverRender = async (req, res, next) => {
    // 이 함수는 404가 떠야 하는 상황에 404를 띄우지 않고 서버 사이즈 렌더링을 해줍니다.
    const context = {};
    ...
    ReactDOMServer.renderToStaticMarkup(jsx); // renderStaticMarkup으로 한번 렌더링합니다.
    try {
    ...
    const stateScript = `<script>__PRELOAD_STATE__=${stateString}</script>`;// 리덕스
    // 초기상태를 스크립트로 주입합니다.
